        ...
        for step, data in enumerate(train_loader):
            ...
            if step % 100 == 0:
                print('Epoch: ', epoch, 'Step', step,
                        'Train_loss: ', train_loss / ((step + 1) * 20),
                        'Train acc: ', train_acc / ((step + 1) * 20))

        print('Epoch: ', epoch,
        'Train_loss: ', train_loss / len(train_data),
        'Train acc: ', train_acc / len(train_data))

        # save model 每个epoch都保存--------------------------------
        # if (epoch + 1) % 1 == 0:
        #     torch.save(model.state_dict(), '/data/liujiaji/action/vgg16log/output/params_' + str(epoch + 1) + '.pth')
