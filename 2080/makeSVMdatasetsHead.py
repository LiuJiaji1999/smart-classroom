...
target_names = ['headDown','headUp']
print(classification_report(head_actionLabels, test_label,target_names=target_names))
# print(confusion_matrix(head_actionLabels, test_label, labels = target_names))
# print(multilabel_confusion_matrix(head_actionLabels, test_label,labels = target_names))



# 测试 6分类 ['raiseHand', 'sleep', 'stand', 'takeNote', 'unknown', 'usePhone']
print(len(datasets))
...
model.train(x_train,y_train,save_path='/data/liujiaji/action/kedaclassroom/kedasvmpkl/kedasvm_head_weights.pkl')
model.eval(x_train,y_train,report_path='/data/liujiaji/action/kedaclassroom/kedasvmpkl/kedasvm_head_weights.pkl')
model.eval(x_test,y_test,report_path='/data/liujiaji/action/kedaclassroom/kedasvmpkl/kedasvm_head_weights.pkl')
...
target_names=['raiseHand', 'sleep', 'stand', 'takeNote', 'unknown', 'usePhone']
...
# print(confusion_matrix(y_test, pre_test, labels = target_names))
# print(multilabel_confusion_matrix(y_test, pre_test,labels = target_names))
...
# 调用 GridSearchCV,将 SVC(), tuned_parameters, cv=5, 还有 scoring 传递进去，
    clf = sklearn.model_selection.GridSearchCV(sklearn.svm.SVC(gamma='auto',decision_function_shape='ovo'), tuned_parameters, cv=5,
