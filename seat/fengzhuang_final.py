...
class ssMatch(object):
    ...
    def aloneSeat(self,seat,seatnum): 
        # _,_,seatnums = self.getSeat()
        # print(seatnum)
        line1 = [seat[0], seat[1]]
 ...
 # 4  映射未检测到的骨骼信息
    ...
    def findSite(self,skeleton_result,seat_result):
        ...
        cv2.imwrite('dangxiao_test/4_allMatch1.png',img)    
        return match
    ...
    def match(self):
        allbox,users,seatnums = self.getSeat()
        tt = []
        img = cv2.imread('dangxiao_test/1_paiSeat.png')
        for i in range(len(allbox)): #6
            # _,users,seatnums = self.getSeat()
            # # print(seatnums[i])
            sseat = self.aloneSeat(allbox[i],seatnums[i])
